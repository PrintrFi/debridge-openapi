...
pub async fn single_swap_controller_v10_get_chain_estimation(
...
) -> Result<models::ChainEstimationResponseV10, Error<SingleSwapControllerV10GetChainEstimationError>>
{
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SingleSwapControllerV10GetChainEstimationError> =
...
pub async fn single_swap_controller_v10_get_chain_transaction(
    configuration: &configuration::Configuration,
    chain_id: &str,
    token_in: &str,
    token_in_amount: &str,
    token_out: &str,
    token_out_recipient: &str,
    slippage: Option<&str>,
    affiliate_fee_percent: Option<f64>,
    affiliate_fee_recipient: Option<&str>,
    sender_address: Option<&str>,
) -> Result<
    models::ChainTransactionResponseV10,
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SingleSwapControllerV10GetChainTransactionError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
